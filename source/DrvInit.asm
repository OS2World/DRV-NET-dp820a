...
Credit		db	cr,lf,' NSC DP83820 Another OS/2 NDIS MAC Driver '
		db	'ver.1.09 (2005-12-24)',cr,lf
Copyright	db	0	; Write copyright message here if you want.

Heap		db	( 147*(9*sizeof(bufdesc) + sizeof(txdesc)) + \
			  252*sizeof(bufdesc) ) dup (0)
...
;	jnz	short loc_e3
	...
	and	cl,110b		; memory type
	cmp	cl,110b		; invalid type
	jz	short loc_e3
	cmp	cl,100b		; 64bit address
	jnz	short loc_irq
	mov	dx,[FH_bdf]
	mov	[si].P_PCI_ReadConfigSpace.Subfunction,PCI_ReadConfigSpace
	mov	word ptr [si].P_PCI_ReadConfigSpace.Bus,dx
	mov	[si].P_PCI_ReadConfigSpace.ConfigRegister,18h	; MEM
	mov	[si].P_PCI_ReadConfigSpace.RegSize,4
	call	IOCtlOEMPCI
	or	ax,ax
	jnz	short loc_e3
	mov	eax,[di].D_PCI_ReadConfigSpace.Data
	or	eax,eax		; bit[63:32] is 0?
	jnz	short loc_e3
...
loc_d:
	shl	ax,4
	and	cfgPauseThresh,11001111b
	or	cfgPauseThresh,al
loc_ex:
	clc
	retn
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwSFLON	; Warning: Invalid PS_STLO.
	jmp	short loc_ex
sci_SFLWON	endp

sci_SFLWOFF	proc	near
	cmp	es:[bp].KeywordEntry.NumParams,1
	jnz	short loc_ce
	cmp	es:[bp].KeywordEntry.cmiParam1.ParamType,0
	jnz	short loc_ce
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwSFLOF	; Warning: Invalid PS_STHI.
	jmp	short loc_ex
sci_SFLWOFF	endp

sci_DFLWON	proc	near
	cmp	es:[bp].KeywordEntry.NumParams,1
	jnz	short loc_ce
	cmp	es:[bp].KeywordEntry.cmiParam1.ParamType,0
	jnz	short loc_ce
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwDFLON	; Warning: Invalid PS_FFLO.
	jmp	short loc_ex
sci_DFLWON	endp

sci_DFLWOFF	proc	near
	cmp	es:[bp].KeywordEntry.NumParams,1
	jnz	short loc_ce
	cmp	es:[bp].KeywordEntry.cmiParam1.ParamType,0
	jnz	short loc_ce
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwDFLOF	; Warning: Invalid PS_FFHI.
	jmp	short loc_ex
sci_DFLWOFF	endp
...
sci_RXAEP	proc	near
	...
	jnz	short loc_w
loc_n:
	and	cfgRxAcErr,not high(highword(AEP))
	jmp	short loc_ex
loc_y:
	or	cfgRxAcErr,high(highword(AEP))
loc_ex:
	clc
	retn
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwRXAE	; Warning: Invalid AcceptErrors
	jmp	short loc_ex
sci_RXAEP	endp
...
sci_RXARP	proc	near
	...
	jnz	short loc_w
loc_n:
	and	cfgRxAcErr,not high(highword(ARP))
	jmp	short loc_ex
loc_y:
	or	cfgRxAcErr,high(highword(ARP))
loc_ex:
	clc
	retn
loc_ce:
...
sci_RXAIRL	proc	near
	...
	jnz	short loc_w
loc_n:
	and	cfgRxAcErr,not high(highword(AIRL))
	jmp	short loc_ex
loc_y:
	or	cfgRxAcErr,high(highword(AIRL))
loc_ex:
	clc
	retn
loc_ce:
...
loc_3:
	...
	jnz	short loc_1
			...
			; terminator to avoid NULL link
	mov	word ptr [bx].bufdesc.cmdsts[2],highword(OWN or MORE)
